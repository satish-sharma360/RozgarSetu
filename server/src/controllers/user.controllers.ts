...
class UserControllers {
  public static async registerUser(req: Request, res: Response) {
    try {
      const { name, email, phone, password, role, location, profileImage } = req.body;
  ...
  public static async loginUser(req: Request, res: Response) {
    try {
      ...
      res.cookie("token", token);
      res.status(200).json({ success: true, message: "User login successfully", data: isUser, token });
    } catch (error) {
  ...
  public static async logOut(req: Request, res: Response) {
    try {
      ...
      res.status(200).json({ success: true, messsage: "User logout successfully" });
    } catch (error) {
  ...
  public static async updateProfile(req: Request, res: Response) {
    try {
      ...
      const { name, phone, skills, experience, isAvailable, location, profileImage } = req.body;
  ...
  public static async updateStatus(req: Request, res: Response) {
    try {
      ...
      if (!loggineduser) return res.status(401).json({ message: "User Not found" });
  ...
  public static async getuserById(req: Request, res: Response) {
    try {
      const { id } = req.params;
      if (!id) return res.status(401).json({ message: "No user with this id" });
  ...
  // NEW: Get all available workers, optionally filtered by skills
  public static async getWorkers(req: Request, res: Response) {
    try {
      ...
      const workers = await User.find(filter)
        .select("name email skills rating totalReviews location isAvailable profileImage")
        .sort({ rating: -1 });
